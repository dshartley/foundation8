...
namespace Smart.Platform.Net.Serialization.JSON
{
    ...
    public class DataJSONWrapper
    {
        ...
        /// <param name="parameterKey">The parameter key.</param>
        /// <returns></returns>
        public bool GetParameterValueBool(string parameterKey)
        {
            #region Check Parameters
            ...
            Boolean     result = (r.ToLower() == "true" || r == "1");
        ...
        /// <returns></returns>
        public DataJSONWrapper Find(string ID)
        {
            DataJSONWrapper result = null;

            // Go through each item
            foreach (DataJSONWrapper item in _items)
            {
                if (item.ID == ID)
                {
                    result = item;
