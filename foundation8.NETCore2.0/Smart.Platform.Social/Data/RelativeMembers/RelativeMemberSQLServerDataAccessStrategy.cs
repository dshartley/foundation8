...
namespace Smart.Platform.Social.Data.RelativeMembers
{
    ...
    public class RelativeMemberSQLServerDataAccessStrategy : SQLServerDataAccessStrategyBase, IRelativeMemberDataAccessStrategy
    {
        ...
        private void DoLoadDataSetTables(IDataItemCollection collection, DataSet data)
        {
            ...
            {
                //// Get ArtworkComments table
                //DataTable t = data.Tables[1];

                //// Go through each row
                //foreach (DataRow r in t.Rows)
                //{
                //    int artworkID = int.Parse(r["ArtworkID"].ToString());
                //    string datePosted = r["DatePosted"].ToString();
                //    string postedByName = r["PostedByName"].ToString();
                //    string text = r["Text"].ToString();

                //    // Find the artwork item
                //    ArtWork a = (ArtWork)collection.GetItem(artworkID);

                //    if (a != null)
                //    {
                //        // Set the values in the artwork item
                //        a.LatestArtworkCommentDatePosted = DateTime.Parse(datePosted);
                //        a.LatestArtworkCommentPostedByName = postedByName;
                //        a.LatestArtworkCommentText = text;
                //    }
                //}
            }
        }
